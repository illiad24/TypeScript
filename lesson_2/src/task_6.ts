...
function getMarkersList(markersNumber: number): Marker[] {
  ...
  for (let i = 0; i < markersNumber; i++) {
    const input = prompt(
      `Маркер ${i + 1}: Введіть lat, long та назву міста через пробіл`
    );
    if (input) {
...
function userSearch(): void {
  ...
  if (input) {
    const data = input.split(" ");
    const searchResult = getClosestPlace(
      Number(data[0]),
      Number(data[1]),
      markersList
    );
    alert(
      `Найближче місто: ${searchResult[2]} (Координати: ${searchResult[0]}, ${searchResult[1]})`
    );
    console.log("Результат:", searchResult);
  }
}

function getClosestPlace(lat: number, long: number, list: Marker[]): Marker {
